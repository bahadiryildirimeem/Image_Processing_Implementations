function [processedImage] = gaussianLowPassFilter(imageName, filterD)
...
gaussianLowPassFilter = zeros(nofRow, nofColumn);
...
for i=1:nofRow
    for j=1:nofColumn
      gaussianLowPassFilter(i,j) = exp(-1*((euclideanDistances(i,j)^2) / (2*filterDia^2)));
    end
end

processedFFT = gaussianLowPassFilter .* F1C;
