function [processedImage] = idealBandRejectFilter(imageName, filterD, filterW)
...
idealBandRejectFilter = ones(nofRow, nofColumn);
...
for i=1:nofRow
    for j=1:nofColumn
        if((euclideanDistances(i,j) >= (filterDia - filterWidth/2)) &&  (euclideanDistances(i,j) < (filterDia + filterWidth/2)))
            idealBandRejectFilter(i,j) = 0;
        end
    end
end

processedFFT = idealBandRejectFilter .* F1C;
