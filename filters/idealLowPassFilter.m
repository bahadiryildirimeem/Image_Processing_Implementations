function [processedImage] = idealLowPassFilter(imageName, filterD)
...
idealLowPassFilter = zeros(nofRow, nofColumn);
...
for i=1:nofRow
    for j=1:nofColumn
        if(euclideanDistances(i,j) <= filterDia)
            idealLowPassFilter(i,j) = 1;
        end
    end
end

processedFFT = idealLowPassFilter .* F1C;
