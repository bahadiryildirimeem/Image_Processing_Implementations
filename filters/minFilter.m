function [processedImage] = minFilter(imageName, recSize)
...
minFilterRecSize = recSize;
for i=1:nofColumn
    for j=1:nofRow
        pixVal = zeros(1, minFilterRecSize^2);
        counter = 0;
        for k=-1*(minFilterRecSize-1)/2:1:(minFilterRecSize-1)/2
            for l=-1*(minFilterRecSize-1)/2:1:(minFilterRecSize-1)/2
                counter = counter + 1;
                indexX = l+j;
                indexY = k+i;
                  if indexX < 1
