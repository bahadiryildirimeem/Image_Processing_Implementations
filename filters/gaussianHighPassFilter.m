function [processedImage] = gaussianHighPassFilter(imageName, filterD, filterDeg)
...
gaussianHighPassFilter = zeros(nofRow, nofColumn);
...
for i=1:nofRow
    for j=1:nofColumn
      gaussianHighPassFilter(i,j) = 1-exp(-1*((euclideanDistances(i,j)^2) / (2*filterDia^2)));
    end
end
processedFFT = gaussianHighPassFilter .* F1C;
