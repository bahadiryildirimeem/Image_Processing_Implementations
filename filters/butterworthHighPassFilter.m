function [processedImage] = butterworthHighPassFilter(imageName, filterD, filterDeg)
...
butterworthHighPassFilter = zeros(nofRow, nofColumn);
...
for i=1:nofRow
    for j=1:nofColumn
      butterworthHighPassFilter(i,j) = 1 / (1+(filterDia / euclideanDistances(i,j))^(2*butterworthDegree));
    end
end

processedFFT = butterworthHighPassFilter .* F1C;
